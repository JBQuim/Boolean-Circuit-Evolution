...
for j in range(repetitions):
    ...
    with open("Data/ModularGoals/Networks" + str(j) + ".txt", "w") as outputFile:
        networkStrings = [",".join([str(k) for k in network]) for network in winners[:i]]
        outputString = "\n".join(networkStrings)
        outputFile.write(outputString)

    with open("Data/ModularGoals/Fitness" + str(j) + ".txt", "w") as outputFile:
        outputString = "\n".join([str(k) for k in history[:i]])
        outputFile.write(outputString)

    with open("Data/ModularGoals/Modularity" + str(j) + ".txt", "w") as outputFile:
